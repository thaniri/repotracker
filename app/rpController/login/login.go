...
func checkPasswordHash(email string, password string) bool {
	query := "SELECT passwordHash from rpUsers WHERE email=?"
	passwordHash := []byte(rpDatabase.SelectQueryOneRow(query, email))
	err := bcrypt.CompareHashAndPassword(passwordHash, []byte(password))
	if err != nil {
		rpLogger.Logger.Error("Passwords don't match", err)
		return false
	}
	return true
}
